        ...
        <TableBody className={classes.tbody}>
          {props.avails.map((avail) => (
            <TableRow key={avail.teacher_availability_id}>
              <TableCell align="center">{`${moment(avail.start_time_stamp, 'YYYY-MM-DDTHH:mm:ss Z').format('dddd')}s at ${moment(avail.start_time_stamp, 'YYYY-MM-DDTHH:mm:ss').format('h:mm a')}`}</TableCell>
